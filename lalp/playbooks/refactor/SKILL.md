...
Same behavior, better shape. The contract is frozen (`../../principles/preserve-behavior/SKILL.md`): every deviation is either a named decision or a bug.
...
Small steps, each leaving the suite green: extract → move → invert, one commit-sized move at a time (`../../principles/sequence-units/SKILL.md`). Between moves, run the tests. Every time, no exceptions.
...
Full suite green, plus behavior spot-checks on the real artifact where feasible (`../../disciplines/prove-it-works/SKILL.md`). Diff review: the change touches shape, not behavior — anything behavioral in the diff is named and justified.
