...
Same change, many sites. Hands are for one site; levers are for many (`lalp-build-the-lever`).
...
Apply. Build, tests, lint — green. Missed sites mean the inventory was wrong: fix the *search*, not just the site (`lalp-root-cause` applies to tooling too). Re-run until clean.
