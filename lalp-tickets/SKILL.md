...
Implementation runs one ticket at a time through the task playbook (`lalp-feature`, `lalp-bugfix`) with `lalp-tdd` per ticket, and `lalp-code-review` before each commit. Finish line: every acceptance criterion from the spec is satisfied by some ticket.
