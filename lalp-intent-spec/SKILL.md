...
Re-read what the user asked. Ask 2-3 sharp questions only where something material is ambiguous (outcomes, scope edges, conflicting priorities). Don't run a full interview — most of the contract comes from the request itself. Deep ambiguity is `lalp-grill`'s job, and it hands off here.
...
Follow `lalp-understand` first if you don't fully know the flow the spec touches. Write the spec in plain language, through `lalp-unslop` — it is read by humans and agents alike.
...
- Tickets are split from it and cite its criteria by name (`lalp-tickets`).
- Implementation is traceable to it — every hunk answers to a criterion, with `lalp-tdd` at the seams.
- Review checks it criterion by criterion (`lalp-code-review`, Spec axis).
