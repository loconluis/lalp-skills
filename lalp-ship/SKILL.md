...
Run the artifact the way a user would: build it, walk the acceptance criteria one by one, re-run the original reproduction for bugfixes. `lalp-prove-it-works` — the evidence must match what changed. A green build alone never ships anything.
...
- Commit message: what and why, imperative, no filler — written through `lalp-unslop`.
