...
Follows `lalp-root-cause` end to end: reproduce → cause → mechanism → evidence.
...
When feasible, encode the reproduction as a failing test (`lalp-tdd`, red). Feasibility is named, not assumed — some bugs only reproduce against live systems, and that's a fact to record, not a step to skip silently.
...
`lalp-smallest-change`. No nil-guards that silence the crash. No retry loop wrapped around a broken thing. The fix removes the cause; it doesn't sedate the symptom.
