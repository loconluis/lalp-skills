...
Genuinely uncertain? N parallel attempts via subagents — one per approach, same question. Different strategies and models welcome. The comparison is the point; effort per attempt stays small. A *design* fork (shape, not data) goes to `lalp-architect` Phase B instead — the arena for shapes.
...
Prototypes don't get "improved into production". Production work starts fresh through `lalp-feature`, carrying what was learned.
